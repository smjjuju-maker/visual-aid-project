#!/usr/bin/env python3
"""
gen_flow_both.py  — <횡단보도+점자블록 함께 시나리오> 테스트 음성을 하나의 mp3로.
멘트는 코드(step_converter.py)의 환경조회(점자블록→횡단보도 순) 문구 그대로 사용.

실행:  python src/gen_flow_both.py   →  flow_both.mp3
필요:  pip install gTTS pydub  +  ffmpeg  (인터넷 필요)
"""
import os, sys, tempfile

OUT_FILE = "flow_both.mp3"
BITRATE = "192k"
LEAD_IN_SILENCE_SEC = 1.0     # 맨 앞 무음(블루투스 첫 음절 끊김 방지)
BUTTON_GAP = 3.0              # 버튼(=Enter) 누른 뒤 안내 전 간격(초)

# (앞 무음 초, 문장) — 코드 문구 그대로 (점자블록 → 횡단보도 순, 공백으로 이어짐)
FLOW = [
    (0.0,        "안내를 시작합니다."),
    (BUTTON_GAP, "열두 시 방향 세 걸음 앞에 점자블록. 한 시 방향에 횡단보도."),  # ← 버튼 누른 후
]
