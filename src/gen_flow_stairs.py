#!/usr/bin/env python3
"""
gen_flow_stairs.py  — <계단 시나리오> 테스트 음성을 하나의 mp3로 만든다.
멘트는 코드(step_converter.py)에 정해진 문구 그대로 사용.

실행:  python src/gen_flow_stairs.py   →  flow_stairs.mp3
필요:  pip install gTTS pydub  +  ffmpeg  (인터넷 필요)
"""
import os, sys, tempfile

OUT_FILE = "flow_stairs.mp3"
BITRATE = "192k"
LEAD_IN_SILENCE_SEC = 1.0     # 맨 앞 무음(블루투스 첫 음절 끊김 방지)
DEFAULT_GAP = 2.5             # 안내 사이 기본 간격(초)

# (앞 무음 초, 문장) — 코드 문구 그대로
FLOW = [
    (0.0,         "안내를 시작합니다."),
    (DEFAULT_GAP, "주의. 정면 네 걸음 앞에 장애물. 양옆으로 피할 수 있음."),
    (DEFAULT_GAP, "주의, 앞에 내려가는 계단."),
    (DEFAULT_GAP, "정지. 장애물. 오른쪽으로 이동."),
    (DEFAULT_GAP, "주의, 앞에 내려가는 계단."),
]
